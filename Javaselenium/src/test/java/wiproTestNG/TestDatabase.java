...
public class TestDatabase {
	private static final org.apache.logging.log4j.Logger log = LoggerHelper.getLogger(TestDatabase.class);
	@Test
	...
	// fetch the db url , password and username of the my sql db 
		...
		String query = "select * from peeps";
		// pass the query to result set
		ResultSet rs = st.executeQuery(query);
		while (rs.next()) {
			String usrname = rs.getString("FName");
			String paswd = rs.getString("LName");
